...
function TLV_Spectral_Calculations()
clear; close all;

%INPUTs
dirs.d = '/Users/brentfisher/Documents/MATLAB';
LampFile = '/KrCl_spec0.csv';       %spectrum: KrCl
% LampFile = '/KrBr_spec1.csv';     %spectrum: KrBr
LampDose_mJcm2_at_Peak = 200;        %assume this is dose of the (filtered) *Peak Only*
...
TLV.mWcm2.old   = tmp(:,2);
TLV.mWcm2.new   = tmp(:,3);
i270            = find(TLV.nm == 270);
TLV.S.old       = tmp(i270,2)./tmp(:,2);
TLV.S.new       = tmp(i270,3)./tmp(:,3);
...
TLV.lampwfilt.relPow  = TLV.lamp.relPow .* ( abs(TLV.nm - lamp.peak_nm) <= FilterWidth_nm/2 );
