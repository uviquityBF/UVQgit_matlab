...
function batch_analyze_spectra_multi_type()
    ...
    for k = 1:num_files
        ...
        % --- Read Numeric Data + Integration Time ---
        int_time = 100; % Default to target for raw CSV to keep scale factor 1
        switch type
            case 'Avantes'
                DAT = read_spectrum_file(full_path, 'avantes');
                WL = DAT.wl;
                Intensity = DAT.counts;
                if ~isnan(DAT.Tint); int_time = DAT.Tint; end % Avantes header is already in ms
            case 'OceanOptics'
                DAT = read_spectrum_file(full_path, 'oceanoptics');
                WL = DAT.wl;
                Intensity = DAT.counts;
                if ~isnan(DAT.Tint); int_time = DAT.Tint * 1000; end % sec to ms
            case 'CSV_2Col'
