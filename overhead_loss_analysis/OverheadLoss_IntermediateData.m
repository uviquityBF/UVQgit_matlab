...
function OverheadLoss_IntermediateData_Batch_ManyFiles(dirs)
    ...
    %[0]
    rootFolder = dirs.d;
    [fileStruct, ~, matData] = load_intermediate_mats(rootFolder);
...
%         do_plot=1;
        ...
        for ik = 1:length(k_sel)  %loop over repetitions of same device
            ...
            D = matData{k};
            x_index_array = D.x_index_array;   y_data_array = D.y_data_array;
            x_iqr = D.x_iqr;   y_iqr = D.y_iqr;   y_filtered = D.y_filtered;
            fit_region_x = D.fit_region_x;   fit_region_y = D.fit_region_y;
            hdrNormalized = D.hdrNormalized;
            input_facet = D.input_facet;   output_facet = D.output_facet;
