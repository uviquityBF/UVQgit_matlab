function Analyze_LIV_Spectra()
    ...
    for i = 1:numFiles
        ...
        %% --- PEAK IDENTIFICATION & IMPROVED CENTROID ---
        [maxVal, maxIdx] = max(int_seg);
        max_ints(i) = maxVal;
        
        if (maxVal > auto_thresh) && (maxIdx > 1) && (maxIdx < length(int_seg))
            % Centroid threshold relative to baseline
            cutOff = avg_noise + (maxVal - avg_noise) * 0.5;
            
            peakMask = int_seg >= cutOff;
            wl_for_centroid = wl_seg(peakMask);
            int_for_centroid = int_seg(peakMask);
            
            cwl = sum(wl_for_centroid .* int_for_centroid) / sum(int_for_centroid);
            center_wls(i) = cwl;
