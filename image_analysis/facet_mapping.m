%% Waveguide Chip Automated Alignment Tool (Unified: facet_mapping3 + facet_mapping4)
addpath('G:\My Drive\Analyses(BF)\Matlab\UVQgit\helpers\image');
clc; clear; close all;

%% --- USER SETTING: rigid-body fit mode ---
% 'independent_per_facet' (default, facet_mapping4 behavior -- the more general case):
%     fits rotation R from the input-stage points only, then computes SEPARATE
%     translations for the input facet and the output facet. Tolerates a real
%     facet-to-facet offset beyond the uniform chip-length correction.
% 'input_stage_only' (facet_mapping3 behavior):
%     fits BOTH rotation and translation using only the input-stage points,
%     then applies that single rigid-body transform to the output facet too.
fit_mode = 'independent_per_facet';
...
%% 7. Mathematical Matrix Alignment (SVD Rigid-Body Fit)
% Rotation always comes from the input-stage points only, in both fit modes.
...
if num_measured == 1
    R = eye(3);
    switch fit_mode
        case 'input_stage_only'
            T = Q_measured_in(1, :) - P_layout_in(1, :);
            T_in = T; T_out = T;
        case 'independent_per_facet'
            T_in  = Q_measured_in(1, :) - P_layout_in(1, :);
            T_out = measured_out_global(1, :) - (corresponding_layout_out(1, :) * R');
        otherwise
    ...
    switch fit_mode
        case 'input_stage_only'
            % Single rigid-body transform (rotation + translation) fit purely from
            % the input-stage points, then applied to the output facet as-is.
            T = centroid_Q - (centroid_P * R');
            T_in = T; T_out = T;
        case 'independent_per_facet'
            % Same rotation, but each facet gets its own best-fit translation.
            T_in  = mean(measured_in_global  - (corresponding_layout_in  * R'), 1);
            T_out = mean(measured_out_global - (corresponding_layout_out * R'), 1);
        otherwise
            error('Unknown fit_mode: %s', fit_mode);
    end
end
...
pred_global_in  = (layout_in_stage_frame * R') + T_in;
pred_global_out = (layout_out_stage_frame * R') + T_out;
...
title_str1 = {sprintf('Plot 1: Calibrated Physical Space (True Length: %.4f mm) [%s]', true_length, fit_mode), ...
              sprintf('Calculated Tilts -> Yaw: %.3f\x00B0 | Pitch: %.3f\x00B0 | Roll: %.3f\x00B0', euler_deg(1), euler_deg(2), euler_deg(3))};
...
fprintf('Calculated System Tilts (Yaw, Pitch, Roll): [%.3f deg, %.3f deg, %.3f deg]\n', euler_deg(1), euler_deg(2), euler_deg(3));
