...
%     'G:\Shared drives\Corp Main\Engineering\LAB Studies\on Tools\_SYSTEM-2\UV Response Calibration of Detection [cts uJ]\process Cal in matlab\Sys2_Calibratin_2026_05_05 (50um slit)'
    ...
    firstData = read_spectrum_file(fullfile(path3, fileList{1}), 'oceanoptics');
    wl_master = firstData.wl;
...
%% 4. Loop Through Each DUT File
    for i = 1:length(fileList)
        ...
        % Load and Calibrate
        dutData = read_spectrum_file(fullfile(path3, thisFile), 'oceanoptics');
        this_counts = dutData.counts;
        ...
        % KNOWN ISSUE: CAL_weighted assignment below is commented out, so CAL_weighted
        % stays all-zero and everything downstream (CAL_Spectrum_power, Response_DUT) is
        % zero/NaN. Left exactly as found -- not in scope for this extraction pass.
