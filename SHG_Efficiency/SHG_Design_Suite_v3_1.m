function SHG_Design_Suite_v3_1()
% SHG Design Suite v3.1
%
% Changes from v3.0:
%   - Phase matching tuning curves (new Section 6):
%     For a reference case at a selected loss level, sweeps dk over a range
%     covering ±n_lobes sinc lobes and plots normalized SHG efficiency vs Δk.
%     Shows both guided and scattered SHG on separate subplots to highlight
%     the broader phase matching bandwidth of scattered SHG.
%     Δn_eff annotation added (Δn = Δk·λ/(4π)) to connect to COMSOL index data.
%   - dk_tune config block added to Section 1.
%
% Inherited from v3.0:
...
%   - Scattered SHG tracking via RK4 state Y(5)
    ...
    dk_tune.n_lobes    = 3;        % number of sinc lobes shown on each side of peak
    ...
    %% --- 6. RESOLVE DISPERSION FOR PHASE MATCHING TUNING ---
    % Looks for dispersion_table.json in the same folder as this script.
    % If found, interpolates dn/dλ for the reference case geometry.
    % If not found, presents hardcoded median values for user confirmation.
    ...
    %% --- 7. PHASE MATCHING TUNING CURVES ---
...
function dY = shg_derivs(z, Y, a0, a3t, a3s, g, dk)
    ...
    dA1 = -0.5*a0*A1;
    dA3 = -0.5*a3t*A3 + 1i*g*(A1^2)*exp(-1i*dk*z);   % Wei's sign convention
    dPs = a3s * abs(A3)^2;
...
function generate_pm_tuning_plot(cases, dk_tune, sim, blue_losses)
    ...
    for lj = 1:n_loss_sel
        ...
        if lj == n_loss_sel
            xlabel(ax, '\delta\lambda_{pump} (nm)  [detuning from perfect phase match]', 'FontSize', 10);
        end
    end
