...
%    IN.Type = 'PointSrc_2DLamb';                    %working (analytic)
...
%   IN.Type = 'Tube_Ideal_NearField';            %working;  only valid for very very near field
    ...
    [IN.XX,IN.YY] = getXXYY(Lplot_X,Lplot_Y,Nx);
      % * * * * * * * * * * * * * * * * * * * * * * *
    RES = get_II(IN);   %% GET IRRADIANCE PATTERN
      % * * * * * * * * * * * * * * * * * * * * * * *
...
%     II = IIraw ./ (sum(IIraw(:))*dA) * IN.Power_tot;    %renormalize  (assuming all power in calculated field!!)
    ...
    [ICUBE,Zvals] = get_III_Cube(IN, Nzsteps, Zmin_cm, Zmax_cm, LplotXY);
...
function  [ICUBE,Zvals]  = get_III_Cube(IN0, Nzsteps, Zmin_cm, Zmax_cm, LplotXY)
...
%     [IN0.XX,IN0.YY] = getXXYY(LplotXY,LplotXY,3);       %XY samples
    ...
    for kz = 1:Nzsteps
        IN0.Z = Zvals(kz);           
          % * * * * * * * * * * * * * * * * * * * * * * *
        RES = get_II(IN0);   %% GET IRRADIANCE PATTERN  (mW/cm2)
          % * * * * * * * * * * * * * * * * * * * * * * *    
...
function RES = get_II(IN)
    ...
    switch IN.Type
        ...
        case 'PointSrc_2DLamb'
            RES = get_PointSrc_2DLamb(IN);                        	%working
...
function RES = get_PointSrc_2DLamb_Truncated(IN)            %[1.1]
	...
	betas = atan2d((XX.^2 + YY.^2).^0.5,IN.Z);  %lamp angle = f(x,y)
 	II = IN.Power_tot .* (cosd(betas)) ./ (solidangle_sr * ( IN.Z^2 + YY.^2 + XX.^2) ) ...
                        .* (betas<=IN.lambertian_FWHM/2 );    % THIS APPLIES TRUNCATION at CONE ANGLE
                    
    %fix (special case if no samples are less than cone angle()):
    if sum(betas(:)<=IN.lambertian_FWHM/2 ) == 0
        isel = find(betas(:) == min(betas(:))) ;
        II(isel) = IN.Power_tot .* (cosd(betas(isel)))  ./ (solidangle_sr * ( IN.Z^2 + (YY(isel)/2).^2 + (XX(isel)/2).^2) )  / length(isel);
    end
...
function RES = get_PointSrc_2D_1_over_COS3(IN)                      %[0.XX]
	...
	betas = atan2d((XX.^2 + YY.^2).^0.5,IN.Z);  %lamp angle = f(x,y)
 	II = IN.Power_tot .* ((cosd(betas)).^a) ./ (solidangle_sr * ( IN.Z^2 + YY.^2 + XX.^2) );            
    RES.XX=XX; RES.YY=YY; RES.II=II; RES.dstep = dstep;
end
...
%Working
function RES = get_PointSrc_2DLamb(IN)                      %[1.2]
...
function RES = get_Tile_Analytic_2DLambertian(IN)       %[1.3]
	...
	betas = atan2d(((abs(XX)-Wtile).^2 + (abs(YY)-Ltile).^2).^0.5,IN.Z);  % lamp angle = f(x,y)
    
	isel = find( abs(XX(:))<Wtile/2 & abs(YY(:))<Ltile/2 );      % points that are directuly beneath tile
	betas(isel) = 0; 
 	II = IN.Power_tot .* ((cosd(betas)).^a) ./ (solidangle_sr * ( IN.Z^2 + YY.^2 + XX.^2) + IN.A_tile);
    %return data
    RES.XX=XX; RES.YY=YY; RES.II=II; RES.dstep = dstep;    
...
function  RES = get_PointSrc_1DLambertian_1DUniform(IN) %[1.5]  
	...
	betas_Y = atan2d(YY,IN.Z);  %lamp angle = f(x,y)
	betas_X = atan2d(XX,IN.Z);  %lamp angle = f(x,y)
	II = IN.Power_tot .* ((cosd(betas_Y)).^a).*(abs(betas_X)<=IN.alpha_tube/2)...
...
function RES = get_Ideal_Simple_Tube(IN)                %[1.7]
    ...
    if IN.Z < IN.L_tube/2
        betas_X = atan2d(XX,IN.Z);  %lamp angle = f(x,y) 
        II = zeros(size(XX));
        isel = find( abs(betas_X(:)) < IN.alpha_tube  &  abs(YY(:)) < IN.L_tube/2 );
...
%             solidangle_sr = (2*pi) * (1 / (a+1));       %[sr] equal in all azimuth --> 2pi 
%             Ltile = IN.L_tile;
%             betas = atan2d(((abs(XX)-Ltile).^2 + (abs(YY)-Ltile).^2).^0.5,IN.Z);  % lamp angle = f(x,y)
%             isel = find( abs(XX(:))<sqrt(Ltile) & abs(YY(:))<sqrt(Ltile) );      % points that are directuly beneath tile
%             betas(isel) = 0; 
%             II = IN.Power_tot .* ((cosd(betas)).^a) ./ (solidangle_sr * ( IN.Z^2 + YY.^2 + XX.^2) + Ltile);
%             
%         %Working ....
%         case  'Tile_Convolve_PointSrc_2DLamb' 
%             a = IN.lambertian_a;  %power to raise cosine to in order to get desired FWHM
%             solidangle_sr = (2*pi) * (1 / (a+1));       %[sr] equal in all azimuth --> 2pi 
%             betas = atan2d((XX.^2 + YY.^2).^0.5,IN.Z);  %lamp angle = f(x,y)
%             II_ptsrc = IN.Power_tot .* ((cosd(betas)).^a) ./ (solidangle_sr * ( IN.Z^2 + YY.^2 + XX.^2) );
%             %apply convolution    
%             Tile_z0 = zeros(size(XX));
%             isel = find( abs(XX(:)) <  IN.W_tile  &  abs(YY(:)) < IN.L_tile );
%             Tile_z0(isel) = 1;
...
%             betas_Y = atan2d(YY,IN.Z);  %lamp angle = f(x,y)
%             betas_X = atan2d(XX,IN.Z);  %lamp angle = f(x,y)
%             II_ptsrc =  IN.Power_tot .* ((cosd(betas_Y)).^a).*(abs(betas_X)<=IN.alpha_tube/2)...
%                             ./ (solidangle_sr * ( IN.Z^2 + YY.^2 + XX.^2) );  
%             %apply convolution    
%             Tile_z0 = zeros(size(XX));
%             isel = find( abs(XX(:)) <  IN.W_tile &  abs(YY(:)) <  IN.L_tile );
