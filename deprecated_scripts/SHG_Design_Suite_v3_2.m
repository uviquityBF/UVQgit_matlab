...
%   - Z-dependent loss (optional per case):
%       cases(k).a0_grad_dBcm_mm   pump loss gradient [dB/cm per mm of waveguide]
%       cases(k).a3_grad_dBcm_mm   SH loss gradient   [dB/cm per mm of waveguide]
%     Loss at position z: a(z) = a_center + a_grad * z.  Default = 0 (constant loss).
%     To model directional pumping asymmetry, negate both gradients for the
%     reverse-pumped case.  Scattered SH gradient scales by the same ratio as
%     the center values (preserves scat/abs split along the waveguide).
...
%   - Scattered SHG tracking via RK4 state Y(5)
    ...
    cases(1).dk_grad        = 0;
    cases(1).a0_grad_dBcm_mm  = -10;   % pump loss gradient [dB/cm per mm of waveguide]
    cases(1).a3_grad_dBcm_mm  = 0;   % SH loss gradient  [dB/cm per mm of waveguide]
...
function [Pg, Ps] = run_core_simulation(cfg)
    ...
    % Loss gradients: dB/cm per mm  ->  Np/um per um
    a0_grad  = (cfg.phys.a0_grad_dBcm_mm / 4.3429) * 1e-7;
    a3t_grad = (cfg.phys.a3_grad_dBcm_mm / 4.3429) * 1e-7;
    if a3t > 0
        a3s_grad = a3t_grad * (as / a3t);   % scale gradient by scattered fraction
    else
        a3s_grad = 0;
    end
...
function [Pg_end, Ps_end] = rk4_engine(L, dz, g, a0, a0_grad, a3t, a3t_grad, a3s, a3s_grad, dk_center, dk_grad, Pp)
    ...
    for i = 1:(length(z_vec) - 1)
        z  = z_vec(i);
        k1 = shg_derivs(z,       Y,          a0, a0_grad, a3t, a3t_grad, a3s, a3s_grad, g, dk_center, dk_grad);
        k2 = shg_derivs(z+dz/2, Y+k1*dz/2,  a0, a0_grad, a3t, a3t_grad, a3s, a3s_grad, g, dk_center, dk_grad);
        k3 = shg_derivs(z+dz/2, Y+k2*dz/2,  a0, a0_grad, a3t, a3t_grad, a3s, a3s_grad, g, dk_center, dk_grad);
        k4 = shg_derivs(z+dz,   Y+k3*dz,    a0, a0_grad, a3t, a3t_grad, a3s, a3s_grad, g, dk_center, dk_grad);
...
function dY = shg_derivs(z, Y, a0, a0_grad, a3t, a3t_grad, a3s, a3s_grad, g, dk_center, dk_grad)
    A1 = Y(1) + 1i*Y(2);
    A3 = Y(3) + 1i*Y(4);
    ...
    a0_z  = a0  + a0_grad  * z;   % pump loss at position z  [Np/um]
    a3t_z = a3t + a3t_grad * z;   % total SH loss at z       [Np/um]
    a3s_z = a3s + a3s_grad * z;   % scattered SH loss at z   [Np/um]

    dA1 = -0.5*a0_z*A1;
    dA3 = -0.5*a3t_z*A3 + 1i*g*(A1^2)*exp(-1i*phase_accum);   % Wei's sign convention
    dPs = a3s_z * abs(A3)^2;
