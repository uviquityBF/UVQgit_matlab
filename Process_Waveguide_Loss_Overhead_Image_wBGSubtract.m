...
function A = Process_Waveguide_Loss_Overhead_Image_wBGSubtract( )
    %clear & get files
    clear; close all;
    program_name = 'Process_Waveguide_Loss_Image'
    dirs.d = 'G:\Shared drives\Corp Main\Engineering\LAB Studies\on Samples\Run F\AM135\SD42\Standardized Testing\Overhead Loss';
%     cd(dirs.d); [FileName,PathName] = uigetfile({'*.mat';'*.tiff'},program_name);
    ...
    % ADD BG SUBTRACTION HERE
    
    level = graythresh(Itest)
    BW = imbinarize(Itest,level);
    imshow (BW )
